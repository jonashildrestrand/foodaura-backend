...
Stored procedures with typed parameters provide complete SQL injection protection. MariaDB treats all procedure parameters as **values**, not executable SQL — there is no string interpolation or dynamic query construction. None of the stored procedures use `PREPARE`/`EXECUTE` with user-supplied strings, so there is no dynamic SQL path for an injected payload to enter.

The additional constraint that `foodaura_backend` and `foodaura_ai` have no direct table access means that even a hypothetical injection through a future dynamic SQL bug could not bypass the procedure boundary — the connection user has no table privileges to exploit.

---

## Grant Summary

The authoritative source is `db/migrations/R__99_grants.sql`. The list below reflects the current state.
