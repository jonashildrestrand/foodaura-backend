...
New users complete a multi-step onboarding UI (steps 0–4) that is managed entirely in the browser by Alpine.js. No data is sent to the server until the user confirms on the final step. At that point, the frontend fires a single `POST /users` with all collected data; the backend creates the user, household, profile, and session in one DB transaction and returns a session token.
...
sequenceDiagram
    ...
    Note over B: User fills steps 0–4 (client-side only)
    B->>N: POST /users (email, password, display_name, household_name, goal, diet_type, dislikes, invite_email)
    N->>C: HTTP (cert validated)
    C->>C: bcrypt(password)
    C->>DB: BEGIN TRANSACTION
    C->>DB: CALL sp_auth_create_user()
    C->>DB: CALL sp_household_create()
    C->>DB: CALL sp_profile_upsert()
    C->>DB: CALL sp_preference_add_ingredient_dislike() [×N]
    C->>DB: CALL sp_auth_create_session()
    C->>DB: COMMIT
    C->>DB: CALL sp_household_invite() [best-effort, outside transaction]
    C-->>B: 201 Created + Set-Cookie: session=<token>
    B->>N: GET /plan (cookie attached)
```

If the transaction fails at any step it is rolled back and no partial data is written. The invite is sent after the commit so a failed invite never prevents account creation.
...
| **Secure** | `true` when the server is reached over TLS (`r.TLS != nil`); `false` on plain HTTP (local dev) |
| **SameSite** | `Lax` — sent on same-site requests and top-level cross-site navigations (GET) |
| **Path** | `/` |
...
|---|---|---|---|
| `GET` | `/onboarding` | No | Multi-step registration UI — single page, all steps rendered server-side, Alpine.js controls visibility |
| `POST` | `/users` | No | Create user, household, profile, and session in one transaction |
