...
nutritional_profiles (
  ...
  diet_type      ENUM('omnivore', 'vegetarian', 'vegan', 'pescatarian') NOT NULL DEFAULT 'omnivore',
  updated_at     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
)
-- diet_type influences recipe generation and discovery (e.g. vegan excludes all meat/dairy recipes).
...
- **Diet type:** `nutritional_profiles.diet_type` is a coarse dietary filter (`omnivore`, `vegetarian`, `vegan`, `pescatarian`). `sp_recipe_find` and AI generation use this to exclude structurally incompatible recipes. Fine-grained exclusions (e.g. "no mushrooms") are stored per-item in `ingredient_dislikes`.
